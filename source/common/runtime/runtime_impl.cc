...
uint64_t SnapshotImpl::getInteger(absl::string_view key, const envoy::type::v3::FractionalPercent& default_value) const {
  ...
  if (entry == values_.end() || !entry->second.uint_value_) {
    if (default_value.denominator() == envoy::type::v3::FractionalPercent::HUNDRED) {
      ...
      auto denominator = ProtobufPercentHelper::fractionalPercentDenominatorToInt(default_value.denominator());
